...
mod part1 {
    ...
    impl Player {
	fn new(id: i8, initial_pos: u8) -> Player {
	    Player {
		id,
		pos: initial_pos,
		score: 0
	...
	fn turn(&mut self, die: &mut DeterministicDie) -> bool {
	    ...
	    let total_roll: u32 = rolled.iter()
		.copied()
		.map(u32::from)
		.sum();
...
mod part2 {
	...
	if in_rolls == 0 {
	    0
	} else if total_roll < 1 {
	    0
	} else if in_rolls == 1 {
	    if total_roll < 4 && total_roll >= 1 {
		1
	    } else {
		0
	    }
	...
	match cache.get(&state) {
	    ...
	    None => {
		let result = calculate_outcomes_slowly(
		    current_player_state, other_player_state, cache);
