...
fn test_add_larger_example_failing_step_b() {
    ...
    // for test_add_larger_example.
    assert_eq!(
        evaluator.explode(SnailNum::from(
            "[[[[12,12],[6,14]],[[15,0],[17,[8,1]]]],[2,9]]"
        )),
        (
            SnailNum::from("[[[[12,12],[6,14]],[[15,0],[25,0]]],[3,9]]"),
            true
        )
    );
}
...
impl Evaluator {
    ...
    fn magnitude(&self, n: SnailNum) -> u64 {
        ...
        let mag_step = |caps: &Captures| -> String {
            ...
            match (
                left.parse::<u64>().map(|l| l.checked_mul(3)),
        ...
        loop {
            ...
            let updated = self
                .magnitude_step_rx
                .replace(digits.as_str(), mag_step)
                .to_string();
            if updated == digits {
                ...
                match result_as_str.parse() {
                    ...
                    Err(e) => {
                        panic!(
                            "magnitude: cannot convert final result {}: {}",
                            &result_as_str, e
                        );
                    }
                }
        ...
        S: Into<SnailNum>,
    {
        ...
        match it.next() {
            Some(first) => it.fold(first, |acc, item| {
...
fn test_magnitude() {
    ...
    assert_eq!(
        evaluator.magnitude(SnailNum::from("[[1,2],[[3,4],5]]")),
        143
    );
    assert_eq!(
        evaluator.magnitude(SnailNum::from(
...
fn test_final_part1_example() {
    ...
    assert_eq!(
        sum,
        SnailNum::from("[[[[6,6],[7,6]],[[7,7],[7,0]]],[[[7,7],[7,7]],[[7,8],[9,9]]]]")
    );
...
fn part1(nums: &[SnailNum]) {
    ...
    println!(
        "Day 18 part 1: {}",
        eval.magnitude(eval.add_snail_numbers(nums.iter().map(|n| n.to_owned())))
    );
}
